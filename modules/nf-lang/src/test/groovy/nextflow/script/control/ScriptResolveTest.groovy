...
class ScriptResolveTest extends Specification {
    ...
    def 'should not resolve an included function as a variable' () {
        given:
        def root = tempDir()
        def module = 'def basename(x) { \'hi\' }\n'

        // an implicit declaration in a process/workflow shadows an included name
        when:
        ...
        def errors = check(root.resolve('a'), [a, root.resolve('a/utils.nf')])
        then:
        errors.size() == 0
        ...
        errors = check(root.resolve('b'), [b, root.resolve('b/utils.nf')])
        then:
        errors.size() == 1
        errors[0].getStartLine() == 5
    ...
    def 'should resolve an included process as a variable' () {
        given:
        def root = tempDir()
